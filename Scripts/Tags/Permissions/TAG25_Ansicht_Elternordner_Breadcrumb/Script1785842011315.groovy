...
// Nur den Unterordner teilen, nicht den Arbeitsbereich (analog SFS04) - dazu frisch
// ueber die Liste in Arbeitsbereich -> Unterordner navigieren
...
WebUI.setText(findTestObject('Share/Page_Folders - PowerFolder/inputEmail_Share'), secondEmail)
...
TestObject workspaceNotVisible = new TestObject()
workspaceNotVisible.addProperty('xpath', ConditionType.EQUALS, "//a[contains(concat(' ',normalize-space(@class),' '),' pica-name ') and normalize-space(text())='" + workspaceName + "']")
WebUI.verifyElementNotPresent(workspaceNotVisible, 10)

// Die Annahme der Einladung hat uns bereits in den Unterordner navigiert (Klick auf
// den Zeilenlink oben); die Breadcrumb-Pruefung erfolgt daher direkt an dieser Stelle
// 3. Breadcrumb: oberste Ebene zeigt den Arbeitsbereich (siehe
// Object Repository/Folders/return_to_subDir.rs fuer die Konvention "pica-crumb")
WebDriver driver = DriverFactory.getWebDriver()
List<WebElement> crumbs = driver.findElements(By.xpath("//a[contains(@class,'pica-crumb')]"))
assert !crumbs.isEmpty()

// 4./5. Dokument im berechtigten Unterordner ist sichtbar
TestObject docPresent = new TestObject()
docPresent.addProperty('xpath', ConditionType.EQUALS, "//*[contains(@data-search-keys, '" + docName + "')]")

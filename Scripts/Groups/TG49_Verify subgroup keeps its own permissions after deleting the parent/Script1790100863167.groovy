...
// Folder 1 : Can read
changeFolderPermission(Folder_sub, 'Can administrate')
...
verifyFolderPermission(Folder_sub,'Can administrate')

WebUI.click(findTestObject('Groups/Page_Groups - PowerFolder/cancel'))

def btn_3 = findGroup(groupName_parent)
...
def btn_4 = findGroup(groupName_sub)
...
// Le dossier hérité du parent doit disparaître
verifyFolderAbsent(Folder_parent)

// Le dossier propre au sous-groupe doit rester
verifyFolderPermission(Folder_sub, 'Can administrate')

WebUI.comment(
	"Le dossier hérité '${Folder_parent}' a disparu et " +
	"le dossier propre '${Folder_sub}' est toujours présent."
)
...
@Keyword
void verifyFolderAbsent(String folderName) {
	WebDriver driver = DriverFactory.getWebDriver()
	...
	boolean folderDisappeared = wait.until {
		List<WebElement> folders =
			driver.findElements(By.xpath(folderXpath))

		return folders.findAll {
			WebElement folder -> folder.isDisplayed()
		}.isEmpty()
	}
	...
	WebUI.comment(
		"Le dossier hérité '${folderName}' a bien disparu."
	)
}

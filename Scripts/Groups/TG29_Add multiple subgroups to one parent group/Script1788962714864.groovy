...
WebUI.click(
	findTestObject(
		'Groups/Page_Groups - PowerFolder/button_Save'
	)
)

// Trouver la cellule Members du groupe parent
WebElement membersCell = wait.until(
	ExpectedConditions.visibilityOfElementLocated(
		By.xpath(
			"//tr[contains(@data-search-keys, '${groupName_1}')]" +
			"//td[@data-tooltip]"
		)
	)
)

// Poser la souris sur Members pour afficher le tooltip
new Actions(driver)
	.moveToElement(membersCell)
	.perform()

// Attendre que le tooltip soit créé
wait.until({
	String tooltipId = membersCell.getAttribute('aria-describedby')

	return tooltipId != null && !tooltipId.isEmpty()
})

// Récupérer l'identifiant du tooltip
String tooltipId = membersCell.getAttribute('aria-describedby')

// Trouver précisément le tooltip noir
...
println("Contenu du tooltip :\n${tooltipText}")

// Vérifier que le tooltip indique 3 sous-groupes
...
for (String subgroupName : subgroupNames) {
	WebUI.verifyEqual(
		tooltipText.contains(subgroupName),
		true
	)
}

...
WebUI.click(findTestObject('Groups/Page_Groups - PowerFolder/subGroup_conf'))

WebUI.click(findTestObject('Groups/Page_Groups - PowerFolder/remove_subGroup'))

WebUI.delay(2)

WebUI.click(findTestObject('Groups/Page_Groups - PowerFolder/button_Save'))

// Attendre le retour à la liste des groupes
WebElement group1 = new WebDriverWait(driver, java.time.Duration.ofSeconds(10)).until(ExpectedConditions.visibilityOfElementLocated(
        By.xpath("//tr[contains(@data-search-keys, '$groupName_1')]")))

// Poser la souris sur le groupe numéro 1
new Actions(driver).moveToElement(group1).perform()

WebUI.delay(2)

// Trouver le tooltip noir visible
...
println('Tooltip après suppression :\n' + tooltipText)

String escapedGroupName = java.util.regex.Pattern.quote(groupName_2)

// Vérifier que la section Subgroup n’est plus affichée
...
// Vérifier que le groupe supprimé n’est plus présent
WebUI.verifyNotMatch(
	tooltipText,
	'(?s).*' + escapedGroupName + '.*',
	true

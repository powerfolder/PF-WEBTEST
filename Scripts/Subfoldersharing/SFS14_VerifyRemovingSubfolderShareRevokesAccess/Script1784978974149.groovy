...
/*
 * Subfolder Sharing: removing a
 * user's explicit permission on a subfolder via the row-level "Remove" action must revoke their
 * access - they can no longer see or open that subfolder afterwards.
 */

// create the member who will be shared on, then removed from, the subfolder
...
String tlfName = 'SFS14_' + RandomStringUtils.randomAlphanumeric(6)
...
String subFolderName = 'SFS14_Sub_' + RandomStringUtils.randomAlphanumeric(6)
...
// share the (currently open) subfolder directly with the member
...
// log out admin, log in as the member and accept the subfolder invitation, confirming access
...
// re-enter the top-level folder and open the subfolder's share dialog
...
// verify the member no longer appears in the subfolder's share dialog
WebUI.verifyElementPresent(findTestObject('Share/Page_Folders - PowerFolder/share_dialog_memberslist_nothing_to_show'), 10)
WebUI.click(findTestObject('Share/close_button_folder_share_mail'))

// log out admin, log back in as the (now removed) member and verify the subfolder is gone
WebUI.click(findTestObject('My_Account/Overview/Page_Accounts - PowerFolder/Icon_account'))
WebUI.click(findTestObject('My_Account/Overview/Page_Accounts - PowerFolder/lang_Log out'))
WebUI.setText(findTestObject('Login/inputEmail'), memberEmail)
WebUI.setText(findTestObject('Login/inputPassword'), GlobalVariable.Pass)

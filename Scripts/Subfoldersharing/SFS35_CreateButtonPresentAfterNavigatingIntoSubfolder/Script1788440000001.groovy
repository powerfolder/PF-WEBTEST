...
String tlfName = 'SFS35_' + RandomStringUtils.randomAlphanumeric(6)
...
String subAName = 'SFS35_SubA_' + RandomStringUtils.randomAlphanumeric(6)
...
// the create dropdown (#pica-files-create-dropdown) is the SAME component on the flat overview
// (there it creates new top-level folders, legitimately visible to anyone) and inside a folder
// (there it is gated by that folder's own permission) - it is always present in the DOM either
// way, only its visibility toggles. So "absent above the share" cannot be asserted from the flat
// overview via this object; this test focuses on the actual regression instead: the in-folder
// create control must become visible immediately upon entering SubA, with no page reload,
// including after leaving and re-entering (the "stale previous view" case from the bug report)
...
WebElement subARowFirstEntry = findFolder(subAName)
WebUI.executeJavaScript('arguments[0].click()', Arrays.asList(subARowFirstEntry))
...
WebElement subARowSecondEntry = findFolder(subAName)
WebUI.executeJavaScript('arguments[0].click()', Arrays.asList(subARowSecondEntry))

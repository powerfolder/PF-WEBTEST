...
WebUI.verifyElementPresent(findTestObject('Page_Link - PowerFolder/span_Add file'), 5)

assert WebUI.getWindowTitle().equals('Link - PowerFolder')

WebUI.click(findTestObject('Links/Page_Link - PowerFolder/Copy_link'))

String my_clipboard_2 = Toolkit.getDefaultToolkit().getSystemClipboard().getContents(null).getTransferData(DataFlavor.stringFlavor)
WebUI.navigateToUrl(my_clipboard_2)
...
// Always fix path for Windows
String downloadPath = home.replace("/", "\\") + "\\Downloads\\"

WebUI.delay(3)

// Assert ZIP downloaded
assert isFolderDownloaded(downloadPath, folderName)
...
// ---------------------------------------------------------
// ZIP DOWNLOAD CHECKER  (corrected)
// ---------------------------------------------------------

boolean isFolderDownloaded(String downloadPath, String folderName) {

	long timeout = 5 * 60 * 1000     // 5 minutes
	long startTime = new Date().getTime()

	// Always build ZIP filename here
	String zipName = folderName + ".zip"
	File zipFile = new File(downloadPath, zipName)

	println("⏳ Waiting for ZIP: " + zipFile.absolutePath)

	while (true) {

		if (zipFile.exists()) {
			println("✅ ZIP FOUND: " + zipName)
		...
		println("... still waiting (${elapsed / 1000}s)")

		if (elapsed > timeout) {
			println("❌ TIMEOUT — ZIP not found: " + zipName)
			println("🔍 Files currently in: " + downloadPath)

			new File(downloadPath).listFiles().each {
				println(" → " + it.getName())
			}

...
String userAFullName = GlobalVariable.userName + ' ' + GlobalVariable.userLastName

String userBFirstName = 'TF18B'
String userBLastName = RandomStringUtils.randomAlphabetic(6).toLowerCase()
String userBEmail = ('tf18b_' + RandomStringUtils.randomNumeric(6)) + '@qa-automated-webtest.com'
String userBFullName = userBFirstName + ' ' + userBLastName
...
WebUI.setText(findTestObject('Accounts/InputFirstName'), userBFirstName)
WebUI.setText(findTestObject('Accounts/InputLastName'), userBLastName)
...
String tlfName = 'TF18_' + RandomStringUtils.randomAlphanumeric(6)
...
WebUI.verifyElementPresent(memberRow(userBFullName), 10)

WebUI.click(memberDropdownToggle(userBFullName))
WebUI.click(openMemberDropdownRemoveAction())
WebUI.waitForElementVisible(findTestObject('Subfoldersharing/confirmation_dialog_title'), 5)

String confirmationTitle = WebUI.getText(findTestObject('Subfoldersharing/confirmation_dialog_title')).toLowerCase()
WebUI.verifyEqual(confirmationTitle.contains('remove') || confirmationTitle.contains('entfernen'), true)
WebUI.click(findTestObject('Share/Page_Folders - PowerFolder/confirme_handover'))

WebUI.verifyElementNotPresent(memberRow(userBFullName), 10)
WebUI.verifyElementPresent(memberRow(userAFullName), 10)
WebUI.click(findTestObject('Share/close_button_folder_share_mail'))
...
TestObject memberRow(String fullName) {
    TestObject obj = new TestObject()
    obj.addProperty('xpath', ConditionType.EQUALS, "//table[@id='share_table']/tbody/tr/td[2][normalize-space(.)='" + fullName + "']")
    return obj
}

TestObject memberDropdownToggle(String fullName) {
    TestObject obj = new TestObject()
    obj.addProperty('xpath', ConditionType.EQUALS, "//table[@id='share_table']/tbody/tr[td[2][normalize-space(.)='" + fullName + "']]//button[contains(@class,'dropdown-toggle')]")
    return obj
}

TestObject openMemberDropdownRemoveAction() {
    TestObject obj = new TestObject()
    obj.addProperty('xpath', ConditionType.EQUALS, "//table[@id='share_table']//ul[contains(@class,'dropdown-menu') and contains(@class,'show')]//a[contains(.,'Remove') or contains(.,'Entfernen')]")
    return obj
}

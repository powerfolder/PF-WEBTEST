...
WebElement folderLinkAfter = findFolderLink(folderName)

WebUI.executeJavaScript('arguments[0].click()', Arrays.asList(folderLinkAfter))
...
TestObject uploadFailedProgressBar = new TestObject('uploadFailedProgressBar')
uploadFailedProgressBar.addProperty('xpath', ConditionType.EQUALS, "//*[contains(@class,'progress-bar-danger')]")

WebUI.waitForElementVisible(uploadFailedProgressBar, 20)

TestObject uploadFailedCaption = new TestObject('uploadFailedCaption')
uploadFailedCaption.addProperty('xpath', ConditionType.EQUALS, "//span[contains(@class,'pica-progress-caption')]")

String rejectionText = WebUI.verifyElementPresent(uploadFailedCaption, 1, FailureHandling.OPTIONAL) ?
    WebUI.getText(uploadFailedCaption) : '(no caption text found)'

println('New upload was correctly rejected after the quota was lowered below usage. Caption: ' + rejectionText)
...
WebElement findFolderLink(String folderName) {
    ...
    String xp = "//td[2]/span/a[contains(text(),'" + folderName + "')]"

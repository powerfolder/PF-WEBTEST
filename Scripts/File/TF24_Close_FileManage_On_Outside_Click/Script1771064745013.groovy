...
def btn = findManageButton(DocName)

WebUI.executeJavaScript('arguments[0].click()', Arrays.asList(btn))

WebUI.verifyElementPresent(findTestObject('file_objects/document/Page_Open - PowerFolder/file_manage'), 5)

WebDriver driver = DriverFactory.getWebDriver()

TestObject tableTO = new TestObject()
tableTO.addProperty("xpath", ConditionType.EQUALS, "//*[@id='files_files_table']")

WebElement tableEl = WebUiCommonHelper.findWebElement(tableTO, 10)

new Actions(driver).moveToElement(tableEl, 5, 5).click().perform()

WebUI.verifyElementNotVisible(findTestObject('file_objects/document/Page_Open - PowerFolder/file_manage'), FailureHandling.STOP_ON_FAILURE)

WebUI.closeBrowser()


WebElement findManageButton(String fileName) {
    WebDriver driver = DriverFactory.getWebDriver()

    return driver.findElement(By.xpath(('//*[contains(@data-search-keys, \'' + fileName) + '\')]/td[6]/div/a/a'))
}
